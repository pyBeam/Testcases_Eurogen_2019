...
max_range = np.array([np.amax(coordinate_X0) - np.amin(coordinate_X0), np.amax(coordinate_Y0) - np.amin(coordinate_Y0),
                      np.amax(coordinate_Z0) - np.amin(coordinate_Z0)]).max()
Xb = 0.5 * max_range * np.mgrid[-1:2:2, -1:2:2, -1:2:2][0].flatten() + 0.5 * (
            np.amax(coordinate_X0) + np.amin(coordinate_X0))
Yb = 0.5 * max_range * np.mgrid[-1:2:2, -1:2:2, -1:2:2][1].flatten() + 0.5 * (
            np.amax(coordinate_Y0) + np.amin(coordinate_Y0))
Zb = 0.5 * max_range * np.mgrid[-1:2:2, -1:2:2, -1:2:2][2].flatten() + 0.5 * (
            np.amax(coordinate_Z0) + np.amin(coordinate_Z0))
